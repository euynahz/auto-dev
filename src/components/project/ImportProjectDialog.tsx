...
/** Extract the last segment of a path as a project name candidate */
...
export function ImportProjectDialog({ open, onOpenChange }: Props) {
  ...
  const handleDirPathChange = (value: string) => {
    setDirPath(value)
    // Auto-fill project name from path if user hasn't manually set it
    if (!nameManuallySet) {
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className={cn(isHorizontal ? 'sm:max-w-[900px]' : 'sm:max-w-[600px]')}>
        <DialogHeader>
          <div className="flex items-center justify-between">
            <div>
              ...
              <DialogDescription>
                导入本地已有的项目目录，AI Agent 将分析项目结构并自动开发
              </DialogDescription>
            </div>
        ...
        <div className={cn(isHorizontal ? 'flex gap-6' : 'space-y-4', 'py-4')}>
          ...
          {/* Right / Bottom: task prompt textarea */}
          <div className={cn(isHorizontal ? 'w-1/2' : '', 'space-y-2')}>
            ...
            <Textarea
              ...
              rows={isHorizontal ? 14 : 6}
              className="resize-none"
            />
            <p className="text-xs text-muted-foreground">
              告诉 AI Agent 你的目标，否则它可能无法理解你想做什么
            </p>
          </div>
        </div>
