            ...
            <div className="flex items-center gap-1 bg-secondary/50 rounded-lg p-0.5 mr-6">
              <button
                ...
                className={cn(
                  ...
                  layout === 'vertical' ? 'bg-primary text-primary-foreground' : 'text-muted-foreground hover:text-foreground'
                )}
                title="竖向布局"
              >
              ...
              <button
                ...
                className={cn(
                  'p-1.5 rounded-md transition-colors cursor-pointer',
                  layout === 'horizontal' ? 'bg-primary text-primary-foreground' : 'text-muted-foreground hover:text-foreground'
                )}
                title="横向布局"
              >
        ...
        {dirConflict ? (
          <div className="py-4 space-y-4">
            ...
            <div className="flex justify-end gap-2">
              <Button variant="outline" onClick={() => setDirConflict(null)} className="cursor-pointer">
                返回修改
              </Button>
              <Button variant="outline" onClick={() => { setDirConflict(null); doCreate(false) }} className="cursor-pointer">
                保留内容，直接创建
              </Button>
              <Button variant="destructive" onClick={() => { setDirConflict(null); doCreate(true) }} className="cursor-pointer">
                清空目录后创建
              </Button>
            </div>
          </div>
            ...
            <div className={cn(isHorizontal ? 'flex gap-6' : 'space-y-4', 'py-4')}>
              ...
              <div className={cn(isHorizontal ? 'w-1/2 space-y-4' : 'space-y-4')}>
                <div className="space-y-2">
                  ...
                  <Input
                    placeholder="例如：/home/user/projects/my-app（留空则自动生成）"
                    value={dirPath}
                    onChange={(e) => handleDirPathChange(e.target.value)}
                  />
                  <p className="text-xs text-muted-foreground">
                    Agent 将在此目录下工作，留空则在 workspace/ 下自动创建
                  </p>
                </div>
                ...
                <label className="flex items-center gap-2 cursor-pointer">
                  <input
                    type="checkbox"
                    checked={reviewBeforeCoding}
                    onChange={(e) => setReviewBeforeCoding(e.target.checked)}
                ...
                {/* Advanced options — always visible in horizontal mode */}
                  ...
                  <div className={cn('space-y-3', !isHorizontal && 'pl-4 border-l-2 border-border')}>
                    ...
                    <div className="space-y-2">
                      <label className="flex items-center gap-2 cursor-pointer">
                        <input
                          type="checkbox"
                          checked={useAgentTeams}
                          onChange={(e) => setUseAgentTeams(e.target.checked)}
                          className="accent-primary"
                        />
